...
function gameWon() {
  ...
  for (i = 0; i < rows; i++) {
    for (j = 0; j < columns; j++) {
      const horizontalSet = new Set([board[i][j], board[i][j+1], board[i][j+2], board[i][j+3]]);
      const verticalSet = new Set([board[i][j], board[i+1][j], board[i+2][j], board[i+3][j]]);
      if(horizontalSet.size === 1 && !horizontalSet.has(null)) alert(`Player ${currentPlayer.slice(-1)} has won horizontally!`);
      if(verticalSet.size === 1 && !verticalSet.has(null)) alert(`Player ${currentPlayer.slice(-1)} has won vertically!`);
